...
class EnhancedDashboard:
    ...
    def _update_portfolio_data(self) -> None:
        ...
        try:
            ...
            # Get daily P&L from data manager
            daily_pnl = getattr(self.data_manager, 'get_daily_pnl', lambda: 0.0)()
            unrealized_pnl = getattr(self.data_manager, 'get_unrealized_pnl', lambda: 0.0)()
            realized_pnl = getattr(self.data_manager, 'get_realized_pnl', lambda: 0.0)()
            ...
            self.update_portfolio_data(portfolio)
            
        except Exception as e:
            log_error(e, "Failed to update portfolio data")
    
    def _update_market_data(self) -> None:
        """Update market data from data manager."""
        try:
            # Get current market data from data manager
            if hasattr(self.data_manager, 'get_current_market_data'):
                market_data = self.data_manager.get_current_market_data()
                if market_data:
                    self.update_market_data(market_data)
            
        except Exception as e:
            log_error(e, "Failed to update market data")
    
    def _update_system_health(self) -> None:
        """Update system health metrics."""
        try:
            # Get actual bot uptime from data manager
            uptime_hours = getattr(self.data_manager, 'get_uptime_hours', lambda: 0)()
            ...
            # Get actual API metrics from Kraken client
            api_rate_used = getattr(self.kraken_client, 'get_rate_limit_used', lambda: 0)()
            api_rate_max = getattr(self.kraken_client, 'get_rate_limit_max', lambda: 1000)()
            
            # Get WebSocket status
            websocket_connected = getattr(self.kraken_client, 'is_websocket_connected', lambda: False)()
            
            # Get error rate from logger
            error_rate = getattr(self.logger, 'get_error_rate_24h', lambda: 0.0)()
            
            # Get active strategies from data manager
            active_strategies = getattr(self.data_manager, 'get_active_strategies', lambda: ["Enhanced Strategies"])()
            ...
            health = SystemHealth(
                ...
                api_rate_limit_used=api_rate_used,
                api_rate_limit_max=api_rate_max,
                websocket_connected=websocket_connected,
                last_heartbeat=datetime.now(),
                error_rate_24h=error_rate,
                active_strategies=active_strategies,
                trading_enabled=getattr(self.data_manager, 'is_trading_enabled', lambda: True)()
            )
...
<body>
    ...
    <div class="container">
        ...
        <!-- Market Data & Performance Chart -->
        <div class="grid">
            <div class="panel half-width">
                <div class="panel-header">
                    <span class="panel-title">Performance Chart</span>
                    <div>
                        <button class="btn" onclick="changeTimeframe('1h')">1H</button>
                        <button class="btn" onclick="changeTimeframe('4h')">4H</button>
                        <button class="btn btn-primary" onclick="changeTimeframe('1d')">1D</button>
                        <button class="btn" onclick="changeTimeframe('1w')">1W</button>
                    </div>
                </div>
        ...
        <div class="grid">
            <div class="panel half-width">
                ...
                <div id="trades-container" style="max-height: 300px; overflow-y: auto;">
    ...
    <script>
        ...
        function refreshMarketData() {
            fetch('/api/market_data')
                .then(response => response.json())
                .then(data => updateMarketDataDisplay(data))
        ...
        function updateMarketDataDisplay(data) {
            ...
            if (Object.keys(data).length === 0) {
                container.innerHTML = '<div class="metric"><span>No market data available</span></div>';
                return;
            }
            
            for (const [pair, marketData] of Object.entries(data)) {
                const metricDiv = document.createElement('div');
                metricDiv.className = 'metric';
                metricDiv.innerHTML = `
                    <span>${pair}</span>
                    <span class="metric-value">$${marketData.price?.toFixed(2) || 'N/A'}</span>
                `;
        ...
        function refreshTrades() {
            fetch('/api/trades')
                .then(response => response.json())
                .then(data => updateTradesDisplay(data))
        ...
        function updateTradesDisplay(trades) {
            ...
            if (trades.length === 0) {
                container.innerHTML = '<div class="trade-entry"><div>No recent trades</div></div>';
                return;
            }
            
            trades.reverse().forEach(trade => addTradeToDisplay(trade));
        }
        ...
        function addTradeToDisplay(trade) {
            ...
            tradeDiv.innerHTML = `
                <div>
                    <div>${trade.pair} ${trade.side} ${trade.volume} @ $${trade.price?.toFixed(2) || 'Market'}</div>
                    <div style="font-size: 10px; color: #7d8590;">${timestamp} - ${trade.strategy}</div>
                </div>
                <div class="${profitClass}">Fee: $${trade.fee?.toFixed(2) || '0.00'}</div>
            `;
        ...
        function controlBot(action) {
            ...
            .then(response => response.json())
            .then(result => {
                const alertsDiv = document.getElementById('control-alerts');
                const alertClass = result.success ? 'alert-success' : 'alert-error';
                const message = result.success ? result.message : result.error;
                
                alertsDiv.innerHTML = `<div class="alert ${alertClass}">${message}</div>`;
                
                // Refresh system health after control action
                setTimeout(refreshSystemHealth, 1000);
            })
            .catch(error => {
                console.error('Error controlling bot:', error);
                document.getElementById('control-alerts').innerHTML = 
                    '<div class="alert alert-error">Failed to control bot</div>';
            });
        }
        ...
        function changeTimeframe(timeframe) {
            ...
            // This would fetch data for the selected timeframe
        }

        // Log functions
        function refreshLogs() {
            // This would fetch recent logs from the server
            console.log('Refreshing logs...');
        }

        function clearLogs() {
            document.getElementById('logs-container').innerHTML = 
                '<div class="log-entry"><span class="log-timestamp">[Cleared]</span> <span class="log-level-info">[INFO]</span> Logs cleared</div>';
        }
    </script>
</body>
</html>
